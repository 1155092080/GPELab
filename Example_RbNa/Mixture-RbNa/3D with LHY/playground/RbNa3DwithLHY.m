...
Nonlinearity = cell(2);
Nonlinearity{1,1} = @(Phi,X,Y,Z)Beta_coupled(1,1)*abs(Phi{1}).^2 + Beta_coupled(1,2)*abs(Phi{2}).^2 + Etta(1)*(Gamma(1)*abs(Phi{1}).^2 + Gamma(2)*abs(Phi{2}).^2).^(3/2);
Nonlinearity{2,2} = @(Phi,X,Y,Z)Beta_coupled(2,2)*abs(Phi{2}).^2 + Beta_coupled(2,1)*abs(Phi{1}).^2 + Etta(2)*(Gamma(1)*abs(Phi{1}).^2 + Gamma(2)*abs(Phi{2}).^2).^(3/2);
...
Physics3D = Nonlinearity_Var3d(Method, Physics3D, Nonlinearity);
