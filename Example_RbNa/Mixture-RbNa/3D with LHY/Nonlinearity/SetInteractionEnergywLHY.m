%% This function is for setting Interaction energy of Rb Na system
%% Input:
...
function [ NonlinearityEnergy ] = SetInteractionEnergywLHY(Constants, Character, Number, LHY_Q)
%% atom numbers
NNa = Number.Na;
NRb = Number.Rb;
...
if LHY_Q
    NonlinearityEnergy = cell(2);
    NonlinearityEnergy{1,1} = @(Phi,X,Y,Z)1/2*Beta(1,1)*abs(Phi{1}).^2 + 1/2*Beta(1,2)*abs(Phi{2}).^2 + 2/5*Etta(1)*(Gamma(1)*abs(Phi{1}).^2 + Gamma(2)*abs(Phi{2}).^2).^(3/2);
    NonlinearityEnergy{2,2} = @(Phi,X,Y,Z)1/2*Beta(2,2)*abs(Phi{2}).^2 + 1/2*Beta(2,1)*abs(Phi{1}).^2 + 2/5*Etta(2)*(Gamma(1)*abs(Phi{1}).^2 + Gamma(2)*abs(Phi{2}).^2).^(3/2);
    NonlinearityEnergy{1,2} = @(Phi,X,Y,Z) 0;
    NonlinearityEnergy{2,1} = @(Phi,X,Y,Z) 0;
else
    NonlinearityEnergy = cell(2);
    NonlinearityEnergy{1,1} = @(Phi,X,Y,Z)1/2*Beta(1,1)*abs(Phi{1}).^2 + 1/2*Beta(1,2)*abs(Phi{2}).^2;
    NonlinearityEnergy{2,2} = @(Phi,X,Y,Z)1/2*Beta(2,2)*abs(Phi{2}).^2 + 1/2*Beta(2,1)*abs(Phi{1}).^2;
    NonlinearityEnergy{1,2} = @(Phi,X,Y,Z) 0;
    NonlinearityEnergy{2,1} = @(Phi,X,Y,Z) 0;
end
